...
async def get_sender(message: Message, command: CommandObject, state: FSMContext):
    if not command.args:
        await message.answer(f"Для создания кампании для рассылки введите команду /sender и название рассылки")
        return
    await message.answer(f'Начинаем создавать рассылку. Название - {command.args}\r\n\r\n'
                         f'Отправьте мне сообщение, которое будет использовано как рекламное')
    await state.update_data(name_camp=command.args)
    await state.set_state(Steps.get_message)

async def get_message(message: Message, state: FSMContext):
    await message.answer(f'Запомнил сообщение!\r\n' f'Добавлять кнопку?', reply_markup=get_confirm_button_keyboard())
...
async def q_button(call: CallbackQuery, bot: Bot, state: FSMContext):
    if call.data == 'add_button':
        await call.message.answer(f'Отправьте текст, который будет на кнопке', reply_markup=None)
...
async def confirm(message: Message, bot: Bot, message_id: int, chat_id: int, reply_markup: InlineKeyboardMarkup = None):
    ...
    await message.answer(f'Это получившееся сообщение. Подтверждаете?',
...
async def sender_decide(call: CallbackQuery, bot: Bot, state: FSMContext, request: Request, senderlist: SenderList):
    ...
    if call.data == "confirm_sender":
        await call.message.answer(f'Начинаю рассылку {name_camp}', reply_markup=None)
        ...
        await call.message.answer(f'Рассылка прошла успешно [{count}] пользователям')
        await request.delete_table(name_camp)

    elif call.data == "cancel_sender":
        await call.message.answer(f'Отменяю рассылку', reply_markup=None)
